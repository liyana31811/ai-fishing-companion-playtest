...
_COLLECTION_CFG = os.path.join(os.path.dirname(_SAVE), "collection_config.json")
_RARITY_ORDER = ["common", "uncommon", "rare", "epic", "legendary", "mythic"]
_RARITY_LABEL_TO_ID = {"常见": "common", "少见": "uncommon", "稀有": "rare", "史诗": "epic", "传说": "legendary", "神话": "mythic"}

def _default_collection_cfg():
    return {"configured": False, "enabled": True, "min_rarity": "rare", "custom_fish_ids": []}

def _load_collection_cfg():
    if os.path.exists(_COLLECTION_CFG):
        try:
            with open(_COLLECTION_CFG, "r", encoding="utf-8") as f:
                cfg = json.load(f)
            d = _default_collection_cfg(); d.update(cfg); return d
        except Exception:
            return _default_collection_cfg()
    return _default_collection_cfg()

def _save_collection_cfg(cfg):
    try:
        with open(_COLLECTION_CFG, "w", encoding="utf-8") as f:
            json.dump(cfg, f, ensure_ascii=False)
    except Exception:
        pass

def _rarity_meets_threshold(rarity_id, cfg):
    if not cfg.get("enabled"):
        return False
    try:
        return _RARITY_ORDER.index(rarity_id) >= _RARITY_ORDER.index(cfg.get("min_rarity", "rare"))
    except ValueError:
        return False

def _c_collection(args):
    cfg = _load_collection_cfg()
    sub = (args[0].lower() if args else "status")
    if sub == "default":
        cfg.update({"configured": True, "enabled": True, "min_rarity": "rare"})
        _save_collection_cfg(cfg)
        return "🏺 收藏缸方案：默认（稀有及以上自动收藏）。想改门槛用 collection custom <rarity>，rarity 可选：common/uncommon/rare/epic/legendary/mythic。"
    if sub == "off":
        cfg.update({"configured": True, "enabled": False})
        _save_collection_cfg(cfg)
        return "🏺 收藏缸已关闭，不会再提醒收藏。想重新开用 collection default 或 collection custom <rarity>。"
    if sub == "custom":
        r = (args[1].lower() if len(args) > 1 else "")
        if r not in _RARITY_ORDER:
            return "没认出这个稀有度「%s」。可选：%s。例：collection custom epic" % (r, "/".join(_RARITY_ORDER))
    ...
    if not cfg.get("configured"):
        return ("🏺 收藏缸还没设置过。可选：\n"
                "  collection default   —— 用默认方案（稀有及以上自动收藏）\n"
                "  collection off       —— 不用收藏功能\n"
                "  collection custom <rarity>  —— 自定义门槛（common/uncommon/rare/epic/legendary/mythic）")
    if not cfg.get("enabled"):
        return "🏺 收藏缸：已关闭。"
    return "🏺 收藏缸：已启用，门槛「%s」及以上。" % RARITY[cfg.get("min_rarity", "rare")]["label"]
...
def _c_goto(loc_id):
    ...
    was_unlocked = loc_id in S["unlocked_locations"]
    was_here = S.get("location_id") == loc_id
    if not was_unlocked:
    ...
    scene_note = ""
    if not was_unlocked:
        scene_note = "\n🖼️ 新钓点解锁：这是第一次打开【%s】——建议配一张环境/气质参考图，让新地图有登场感。" % loc["name"]
    elif not was_here:
        scene_note = "\n🖼️ 换到钓点【%s】——建议配一张场景参考图，帮助人类感觉真的换地方了。" % loc["name"]
    return "来到【%s】。%s%s%s%s%s" % (loc["name"], loc["description"], char, off, hint, scene_note)
def _c_inv():
    out = []
...
def _c_start():
    ...
    backups = []
    try:
        backups = sorted([x for x in os.listdir(here) if x.startswith("fishing_save_") and x.endswith(".json")])
    except Exception:
        backups = []
    latest = backups[-1] if backups else "暂无"
    cfg = _load_collection_cfg()
    if cfg.get("configured"):
        if cfg.get("enabled"):
            collection = "已确认：开启，门槛 %s 及以上。" % cfg.get("min_rarity", "rare")
        else:
            collection = "已确认：关闭。"
    else:
...
def _run_one(line):
    ...
    try:
        ...
        elif c == "collection": return _c_collection(a)
...
_EXPORT_WARN_THRESHOLD = 15   # 超过这么多回合没标记导出，就在输出里强制提醒
_VISUAL_MARKERS = {
    "📜": "漂流瓶",
    "🧩": "藏宝图碎片",
    "🗺️": "藏宝图集齐/潜水点解锁",
...
def _catch_rarity_from_line(s):
    ...
    if s.startswith("✦"):
        return "rare"
    if "传 说" in s or s.startswith("👑"):
        return "legendary"
    if "神 话" in s or s.startswith("✧"):
        return "mythic"
    return None

def _onboarding_prompt():
    cfg = _load_collection_cfg()
    if cfg.get("configured"):
        return ""
...
def _visual_and_export_reminders(out):
    ...
    调用（非纯信息类指令）都跑，不管调用方是谁、走哪个入口，都躲不掉——这是设计目的，不是 bug。"""
    _load()
    cfg = _load_collection_cfg()
    lines = []
    for raw_line in out.split("\n"):
        s = raw_line.strip()
        matched = False
        for marker, label in _VISUAL_MARKERS.items():
...
def cmd(line=""):
    ...
    特殊指令 __exported__：确认已经把存档导出到外部存储（如 Drive）后调用，重置导出提醒的计数。
    ...
    if not raw:
        out0 = _HELP
        return _drain_warn(out0 + _onboarding_prompt() + "\n" + _state_json())
    subs = [s.strip() for s in re.split(r"[;\n]+", raw) if s.strip()]   # 批量：; 或换行分隔
    first_word = subs[0].split()[0].lower() if subs and subs[0].split() else ""
    is_info_only = len(subs) == 1 and first_word in _INFO_ONLY_CMDS
    if len(subs) > 1:
        run = subs[:_BATCH_MAX]
        ...
        out = _run_one(subs[0])
    _save()
    extra = ("" if is_info_only else _visual_and_export_reminders(out)) + _onboarding_prompt()
    return _drain_warn(out + extra + "\n" + _state_json())   # 末尾统一附一行 📊 状态栏 JSON
