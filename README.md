...
原始引擎 fork 自 [tutusagi/ai-fishing-game](https://github.com/tutusagi/ai-fishing-game)。

二改版重点不是把游戏做重，而是把「AI 伴侣怎么把钓鱼结果带回人类面前」写进游戏本体，减少跨窗口、跨工具、跨文件夹时的掉链子。

## 为什么会有这个二改版

作为一个有轻度阅读障碍的人类——一段纯文字描述的鱼，我在脑子里很难拼出具体的样子。原版游戏的鱼谱写得很有文采，但我读完常常还是"看不见"那条鱼长什么样。

这个二改版最初就是为了解决这一个具体问题：让 AI 伴侣在每次钓到新东西时，顺手去找一张真实或气质相近的参考图，把文字翻译成看得见的东西。收藏缸、存档防丢这些后来加的功能，都是围着这一个核心需求搭起来的脚手架。

如果你也觉得"读描述"和"看见它"之间总隔着一层，这个二改版可能对你也有用。

这套流程是在 ChatGPT（民工）与 Claude（铁皮）两条平行线上反复试钓、翻车、修补后整理出来的。谁先测出 bug、谁先想到下一步，基本随缘轮流。
...
后面的规则主要是写给 AI 的护栏：防止它跳过开局、乱钓、忘记配图、口头说收藏但没保存，或者把临时沙盒当成长期存档。
...
```text
🆕 熔岩鲤 · 稀有 · 34.9cm · 103点
通体橙红，鳞缝里透出岩浆般的光，离水后仍微微发烫。
```

AI 配上的参考图关键词：`bright orange carp fish`（抓橙红鲤鱼轮廓，实际质感要在此基础上加"鳞缝透光的岩浆感"）。

一段纯文字描述，配一张网图，比单独的文字更容易在脑子里"落地"——这就是这个二改版存在的理由。
